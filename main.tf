...
# Reference the signalRoom Confluent Cloud Organization
data "confluent_organization" "env" {}

# Create the Confluent Cloud Environment
resource "confluent_environment" "env" {
    display_name = "signal-intelligence-${var.aws_profile}"
...
# Create the Service Account for the Kafka Cluster data-ingestion API
resource "confluent_service_account" "schema_registry_api" {
    display_name = "demo-${var.aws_profile}-environment-api"
    description  = "Demo Environment API Service Account"
}
...
# Create the Data Ingestion Kafka cluster
resource "confluent_kafka_cluster" "demo_kafka_cluster" {
    display_name = "demo"
...
# Create the Service Account for the Kafka Cluster data-ingestion API
resource "confluent_service_account" "demo_kafka_cluster_api" {
    display_name = "demo-${var.aws_profile}-kafka_cluster_data_ingestion-api"
    description  = "Demo Kafka Cluster API Service Account"
}
...
module "demo_kafka_cluster_api_key_rotation" {
    ...
    #Required Input(s)
    owner = {
        id          = confluent_service_account.demo_kafka_cluster_api.id
        api_version = confluent_service_account.demo_kafka_cluster_api.api_version
        kind        = confluent_service_account.demo_kafka_cluster_api.kind
    }

    resource = {
        id          = confluent_kafka_cluster.demo_kafka_cluster.id
        api_version = confluent_kafka_cluster.demo_kafka_cluster.api_version
        kind        = confluent_kafka_cluster.demo_kafka_cluster.kind
    ...
    # Optional Input(s)
    key_display_name = "Confluent Data Ingestion Kafka Cluster Service Account API Key - {date} - Managed by Terraform Cloud"
    number_of_api_keys_to_retain = var.number_of_api_keys_to_retain
    day_count = var.day_count
}
...
resource "aws_secretsmanager_secret" "schema_registry_api_key" {
    ...
    description = "signalRoom Intelligence Schema Registry secrets"
}
...
resource "aws_secretsmanager_secret" "demo_kafka_cluster_api_key" {
    name = "${local.secrets_prefix}/data-ingestion-kafka-cluster"
    description = "signalRoom Intelligence Data Ingestion Kafka Cluster secrets"
}

resource "aws_secretsmanager_secret_version" "demo_kafka_cluster_api_key" {
    secret_id     = aws_secretsmanager_secret.demo_kafka_cluster_api_key.id
    secret_string = jsonencode({"api_key": "${module.demo_kafka_cluster_api_key_rotation.active_api_key.id}", 
                                "api_secret": "${module.demo_kafka_cluster_api_key_rotation.active_api_key.secret}",
                                "java_client_jaas_configuration": "org.apache.kafka.common.security.plain.PlainLoginModule required username='${module.demo_kafka_cluster_api_key_rotation.active_api_key.id}' password='${module.demo_kafka_cluster_api_key_rotation.active_api_key.secret}';",
                                "bootstrap_url": "${confluent_kafka_cluster.demo_kafka_cluster.bootstrap_endpoint}"
                                "rest_endpoint": "${confluent_kafka_cluster.demo_kafka_cluster.rest_endpoint}"})
}
